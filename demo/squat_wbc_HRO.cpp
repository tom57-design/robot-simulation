...
mjModel *mj_model = mj_loadXML("../models/HRO-c2-v1/mjcf/scene_fixed_feet.xml", 0, error, 1000);
...
int main(int argc, const char **argv)
{
    ...
    WBC_priority_HRO WBC_solv(kinDynSolver.model_nv, 6 + 12, 22, 0.7, mj_model->opt.timestep); // WBC solver
    ...
    // variables ini
    double stand_legLength = 1.05; // desired baselink height
    ...
    double startStandingTime_1 = 4.5;
    double startSquatingTime_2 = 9.5;
    double startStandingTime_2 = 12.0;
    ...
    while (!glfwWindowShouldClose(uiController.window))
    {
        ...
        while (mj_data->time - simstart < 1.0 / 60.0 && uiController.runSim) // press "1" to pause and resume, "2" to step the simulation
        {
            ...
            else if (simTime > startSquatingTime_2 && simTime <= startStandingTime_2)
            {
                ...
                RobotState.base_pos_stand = RobotState.base_pos;
                RobotState.base_pos_des(0) = 0;
                RobotState.base_pos_des(1) = 0;
