...
int main(int argc, char **argv)
{
    ...
    while (!glfwWindowShouldClose(uiController.window))
    {
        ...
        while (mj_data->time - simstart < 1.0 / 60.0 && uiController.runSim)
        {
            ...
            Jac_stand.block(0, 0, 6, 12) = RobotState.J_l.block(0, model_nv - 23, 6, 12);
            Jac_stand.block(6, 0, 6, 12) = RobotState.J_r.block(0, model_nv - 23, 6, 12);
            ...
            if (simTime <= prepareTime)
            {
                ...
                if (print_counter % 100 == 0)
                {
                    ...
                    std::cout << "RobotState.base_rot: \n"
                              << RobotState.base_rot << std::endl;
                }
                print_counter++;
            }
            else if (simTime < startJumpingTime && simTime > prepareTime)
            {
                fe_l_pos_L_des(2) = Ramp(fe_l_pos_L_des(2), stand_z, 0.5 * dt); // 0.5
                fe_r_pos_L_des(2) = Ramp(fe_r_pos_L_des(2), stand_z, 0.5 * dt);
                ...
                if (print_counter % 100 == 0)
                {
                    std::cout << "State: Lower Down" << std::endl;
                }
                print_counter++;
            }
            else if (simTime >= startJumpingTime)
            {
                ...
                double jump_vel_des[3] = {0.0, 0.0, sqrt(2.0 * 9.8 * jump_z)};  // Tuned
                double jump_acc_t = 2.0 * (0.55 + stand_z) / (jump_vel_des[2]); // Tuned
                ...
                { // Jump
                    ...
                    L_diag << 50.0, 50.0, 1.0, // eul
                        50.0, 50.0, 200.0,     // pCoM
                        0.1, 0.1, 0.1,         // w
                        0.01, 0.1, 20.0;       // vCoM
                    K_diag << 1.0, 1.0, 0.1,   // fl
                        10.0, 10.0, 10.0,
                        1.0, 1.0, 0.1, // fr
                        10.0, 10.0, 10.0, 1.0;
                ...
                { // up
                    ...
                    if (print_counter % 100 == 0)
                    {
                        std::cout << "State: In the Air" << std::endl;
                    }
                    print_counter++;
                    ...
                    // Eigen::Matrix<double, 31, 1> IKRes;  // For OpenLoong
                    ...
                    Eigen::Matrix<double, 23, 1> IKRes; // For G1_23dof
                    IKRes = resLeg.jointPosRes + resHand.jointPosRes;
                    // For ankle pitch
                    IKRes(model_nv - 6 - 19) = IKRes(model_nv - 6 - 19) + RobotState.base_rpy(1); // + 1.0 / 180.0 * 3.1415926;
                    IKRes(model_nv - 6 - 13) = IKRes(model_nv - 6 - 13) + RobotState.base_rpy(1); // + 1.0 / 180.0 * 3.1415926;
                ...
                { // down
                    ...
                    if (print_counter % 100 == 0)
                    {
                        std::cout << "State: Falling Down" << std::endl;
                    }
                    print_counter++;
                    ...
                    fe_l_pos_W_des[0] = Ramp(fe_l_pos_W_des[0], RobotState.pfeW0[0] + 0.05, fabs(10.0 * dt)); // Tuned
                    fe_r_pos_W_des[0] = Ramp(fe_r_pos_W_des[0], RobotState.pfeW0[3] + 0.05, fabs(10.0 * dt)); // Tuned
                ...
                else if (jump_state == 5)
                {
                    ...
                    RobotState.js_pos_des(2) = Ramp(RobotState.js_pos_des(2), 0.45, 0.5 * dt); // Tuned
            ...
            if (mpc_force.get_ENA())
            {
                ...
                for (int i = 0; i < 12; i++)
                {
                    ...
                    pvtCtr.disablePV(model_nv - 6 - 23 + i);                   // For G1
                    RobotState.motors_tor_des[model_nv - 6 - 23 + i] = Uje(i); // For G1
                }
            }
            else
            {
