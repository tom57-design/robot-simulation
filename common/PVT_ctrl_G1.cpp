...
PVT_Ctr_G1::PVT_Ctr_G1(double timeStepIn, const char *jsonPath) {
...
void PVT_Ctr_G1::dataBusRead(DataBus &busIn) {
    for (int i=0;i<jointNum;i++)
    {
...
void PVT_Ctr_G1::dataBusWrite(DataBus &busIn) {
    busIn.motors_tor_out=motor_tor_out;
    busIn.motors_tor_cur=motor_tor_out;
}

void PVT_Ctr_G1::setJointPD(double kp, double kd, const char *jointName) {
...
void PVT_Ctr_G1::calMotorsPVT(double deltaP_Lim) {
    for (int i=0;i<jointNum;i++)
    {
        double tauDes{0};
...
double PVT_Ctr_G1::sign(double in) {
    if (in>=0)
        return 1.0;
    else
        return -1.0;
...
void PVT_Ctr_G1::enablePV() {
    PV_enable.assign(jointNum,1);
}

void PVT_Ctr_G1::enablePV(int jtId) {
    PV_enable[jtId]=1;
}

// Disable PV control item for all joints.
void PVT_Ctr_G1::disablePV() {
    PV_enable.assign(jointNum,0);
}

void PVT_Ctr_G1::disablePV(int jtId) {
    PV_enable[jtId]=0;
}