...
WBC_priority_G1::WBC_priority_G1(int model_nv_In, int QP_nvIn, int QP_ncIn, double miu_In, double dt) : QP_prob(QP_nvIn,
                                                                                                                QP_ncIn)
{
    timeStep = dt;
...
void WBC_priority_G1::dataBusWrite(DataBus &robotState)
{
...
void WBC_priority_G1::computeTau()
{
...
void WBC_priority_G1::computeDdq(Pin_KinDyn_G1 &pinKinDynIn)
{
    // task definition
    /// -------- walk -------------
    {
        ...
        //**** For G1-23DOF ****//
        // hip-l: 0-2, knee-l: 3, ankle-l: 4-5, hip-r: 6-8, knee-r: 9, ankle-r: 10-11,
        // waist-yaw: 12,
        ...
        kin_tasks_walk.taskLib[id].J(0, 11) = 1; // 12: waist yaw
        ...
        kin_tasks_walk.taskLib[id].J.block(0, 18, 6, 1).setZero(); // exculde waist joints
        ...
        // //**** START OF LOW DAMPING  ****//
        // id = kin_tasks_walk.getId("PosRot");
        // kin_tasks_walk.taskLib[id].errX = Eigen::VectorXd::Zero(6);
        // kin_tasks_walk.taskLib[id].errX.block(0, 0, 3, 1) = base_pos_des - q.block(0, 0, 3, 1);
        // if (fabs(kin_tasks_walk.taskLib[id].errX(0)) >= 0.01)
        ...
        // if (kin_tasks_walk.taskLib[id].errX(2) > 0.002)
        // {
        //     kin_tasks_walk.taskLib[id].errX(2) = 0.002;
        // }
        // desRot = eul2Rot(base_rpy_des(0), base_rpy_des(1), base_rpy_des(2));
        // kin_tasks_walk.taskLib[id].errX.block<3, 1>(3, 0) = diffRot(base_rot, desRot);
        ...
        // kin_tasks_walk.taskLib[id].kd = Eigen::MatrixXd::Identity(6, 6) * 10 / 2;
        ...
        // kin_tasks_walk.taskLib[id].dxDes = Eigen::VectorXd::Zero(6);
        // kin_tasks_walk.taskLib[id].kp = Eigen::MatrixXd::Identity(6, 6) * 500;
        // kin_tasks_walk.taskLib[id].kd = Eigen::MatrixXd::Identity(6, 6) * 20 / 2;
        // kin_tasks_walk.taskLib[id].J = Jsw;
        // kin_tasks_walk.taskLib[id].J.block(0, 18, 6, 1).setZero(); // exculde waist joints
        // kin_tasks_walk.taskLib[id].dJ = dJsw;
        // kin_tasks_walk.taskLib[id].dJ.block(0, 18, 6, 1).setZero(); // exculde waist joints
        ...
        double r_hip_pitch = q(13) - qIniDes(13);
        ...
        kin_tasks_walk.taskLib[id].errX = resLeg.jointPosRes.block<10, 1>(13, 0) - q.block<10, 1>(13, 0);
        ...
        kin_tasks_walk.taskLib[id].kp = Eigen::MatrixXd::Identity(10, 10) * 2000; // 100
        kin_tasks_walk.taskLib[id].kd = Eigen::MatrixXd::Identity(10, 10) * 100;
        kin_tasks_walk.taskLib[id].J = Eigen::MatrixXd::Zero(10, model_nv);
        kin_tasks_walk.taskLib[id].J.block(0, 19, 10, 10) = Eigen::MatrixXd::Identity(10, 10);
        ...
        kin_tasks_walk.taskLib[id].errX(0) = 0 - q(19); // 12+7: waist yaw
    ...
    /// -------- stand -------------
    {
        ...
        // kin_tasks_stand.taskLib[id].J.block(0, 22, 12, 3).setZero(); // exculde waist joints
        kin_tasks_stand.taskLib[id].J.block(0, 18, 12, 1).setZero(); // exculde waist joints
        ...
        kin_tasks_stand.taskLib[id].J.block(0, 18, 3, 1).setZero();  // exculde waist joints
        kin_tasks_stand.taskLib[id].J.block(0, 19, 3, 10).setZero(); // exculde arm joints
        ...
        kin_tasks_stand.taskLib[id].J.block(0, 19, 2, 10).setZero();
        ...
        kin_tasks_stand.taskLib[id].J.block(2, 18, 3, 1).setZero();  // exculde waist joints
        kin_tasks_stand.taskLib[id].J.block(2, 19, 3, 10).setZero(); // exculde arm joints
        ...
        kin_tasks_stand.taskLib[id].W.diagonal()(18) = 200; // 12+6: waist yaw
        ...
        kin_tasks_stand.taskLib[id].errX(0) = 0 - q(19); // 12+7: waist yaw
...
void WBC_priority_G1::copy_Eigen_to_real_t(qpOASES::real_t *target, const Eigen::MatrixXd &source, int nRows, int nCols)
{
    int count = 0;
...
void WBC_priority_G1::setQini(const Eigen::VectorXd &qIniDesIn, const Eigen::VectorXd &qIniCurIn)
{
    qIniDes = qIniDesIn;
    qIniCur = qIniCurIn;
}