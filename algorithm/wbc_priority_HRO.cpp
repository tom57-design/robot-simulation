#include "wbc_priority_HRO.h"
#include "iostream"

// QP_nvIn=18, QP_ncIn=22
WBC_priority_HRO::WBC_priority_HRO(int model_nv_In, int QP_nvIn, int QP_ncIn, double miu_In, double dt) : QP_prob(QP_nvIn,
                                                                                                                  QP_ncIn)
{
    timeStep = dt;
    ...
    //******** Added to prevent waist from rotating ********//
    kin_tasks_walk.addTask("fixedWaist");
    ////
    ...
    // //******** Added to prevent waist from rotating ********//
    taskOrder_walk.emplace_back("fixedWaist");
    // ///
...
void WBC_priority_HRO::dataBusRead(const DataBus &robotState)
{
...
void WBC_priority_HRO::dataBusWrite(DataBus &robotState)
{
...
void WBC_priority_HRO::computeTau()
{
...
void WBC_priority_HRO::computeDdq(Pin_KinDyn_HRO &pinKinDynIn)
{
    // task definition
    /// -------- walk -------------
    {
        ...
        //**** For HRO-c2-v1 ****//
        // left hip: 0-2, left knee: 3, left ankle: 4-6
        // right hip: 7-9, right knee: 10, right ankle: 11-13
        // torso: 14-15
        // left shoulder: 16-18, left elbow: 19, left wrist: 20-22
        // right shoulder: 23-25, right elbow: 26, right wrist: 27-29

        kin_tasks_walk.taskLib[id].errX = Eigen::VectorXd::Zero(1);
        kin_tasks_walk.taskLib[id].errX(0) = 0 - q(14); // 14-15: waist
        kin_tasks_walk.taskLib[id].errX(1) = 0 - q(15); // 14-15: waist
        ...
        kin_tasks_walk.taskLib[id].kp = Eigen::MatrixXd::Identity(2, 2) * 200;
        kin_tasks_walk.taskLib[id].kd = Eigen::MatrixXd::Identity(2, 2) * 20;
        kin_tasks_walk.taskLib[id].J = Eigen::MatrixXd::Zero(2, model_nv);

        kin_tasks_walk.taskLib[id].J(0, 14) = 1; // 14-15: waist yaw
        kin_tasks_walk.taskLib[id].J(1, 15) = 1; // 14-15: waist yaw
        ...
        ////
        kin_tasks_walk.taskLib[id].kp.block(3, 3, 1, 1) = Eigen::MatrixXd::Identity(1, 1) * 100 / 50; // Tune for Pz gain
        ////
        ...
        // if (fabs(kin_tasks_walk.taskLib[id].errX(0)) >= 0.06)
        ...
        kin_tasks_walk.taskLib[id].kp = Eigen::MatrixXd::Identity(6, 6) * 10 / 2;
        kin_tasks_walk.taskLib[id].kp.block(3, 3, 3, 3) = Eigen::MatrixXd::Identity(3, 3) * 2000;
        kin_tasks_walk.taskLib[id].kd = Eigen::MatrixXd::Identity(6, 6) * 2 / 2;
        kin_tasks_walk.taskLib[id].kd.block(3, 3, 3, 3) = Eigen::MatrixXd::Identity(3, 3) * 100 / 5;
        kin_tasks_walk.taskLib[id].J = J_base;
        kin_tasks_walk.taskLib[id].dJ = dJ_base;
        kin_tasks_walk.taskLib[id].W.diagonal() = Eigen::VectorXd::Ones(model_nv) * 10;
        ...
        kin_tasks_walk.taskLib[id].ddxDes = Eigen::VectorXd::Zero(6);
        kin_tasks_walk.taskLib[id].dxDes = Eigen::VectorXd::Zero(6);
        kin_tasks_walk.taskLib[id].kp = Eigen::MatrixXd::Identity(6, 6) * 2000;                                  // strict about swing leg's position & orientation
        kin_tasks_walk.taskLib[id].kp.block<1, 1>(2, 2) = kin_tasks_walk.taskLib[id].kp.block<1, 1>(2, 2) * 0.1; // set swing leg's Z direction to be less strict
        kin_tasks_walk.taskLib[id].kd = Eigen::MatrixXd::Identity(6, 6) * 20;                                    // moderate damping
        kin_tasks_walk.taskLib[id].J = Jsw;
        kin_tasks_walk.taskLib[id].J.block(0, 20, 6, 2).setZero(); // exculde waist joints
        kin_tasks_walk.taskLib[id].dJ = dJsw;
        kin_tasks_walk.taskLib[id].dJ.block(0, 20, 6, 2).setZero(); // exculde waist joints
        kin_tasks_walk.taskLib[id].W.diagonal() = Eigen::VectorXd::Ones(model_nv);
        ...
        Eigen::Vector3d hd_l_eul_L_des = {M_PI, -0.00, 0.000};
        Eigen::Vector3d hd_r_eul_L_des = {M_PI, -0.00, 0.000};
        ...
        double l_hip_pitch = q(7) - qIniDes(7);
        double r_hip_pitch = q(14) - qIniDes(14);
        ...
        kin_tasks_walk.taskLib[id].errX = resLeg.jointPosRes.block<14, 1>(16, 0) - q.block<14, 1>(16, 0);
        ...
        kin_tasks_walk.taskLib[id].J.block(0, 22, 14, 14) = Eigen::MatrixXd::Identity(14, 14);
        kin_tasks_walk.taskLib[id].dJ = Eigen::MatrixXd::Zero(14, model_nv);
        kin_tasks_walk.taskLib[id].W.diagonal() = Eigen::VectorXd::Ones(model_nv);

        // std::cout << "Ended HandTrackJoints!" << std::endl;

        ///******** Added to prevent waist rotating ********///
        id = kin_tasks_walk.getId("fixedWaist");

        kin_tasks_walk.taskLib[id].errX = Eigen::VectorXd::Zero(1);

        kin_tasks_walk.taskLib[id].errX(0) = 0 - q(21); // 14+7: waist yaw
        ...
        kin_tasks_walk.taskLib[id].kp = Eigen::MatrixXd::Identity(1, 1) * 20000;
    ...
    /// -------- stand -------------
    {
        ...
        kin_tasks_stand.taskLib[id].J.block(0, 20, 12, 2).setZero(); // exculde waist joints
        ...
        kin_tasks_stand.taskLib[id].J.block(0, 20, 3, 2).setZero();  // exculde waist joints
        kin_tasks_stand.taskLib[id].J.block(0, 22, 3, 14).setZero(); // exculde arm joints
        ...
        kin_tasks_stand.taskLib[id].J.block(0, 20, 1, 2).setZero(); // exclude waist joint

        kin_tasks_stand.taskLib[id].dJ = taskMap * dJ_base;

        kin_tasks_stand.taskLib[id].dJ.block(0, 20, 1, 2).setZero(); // exclude waist joint
        ...
        kin_tasks_stand.taskLib[id].J.block(0, 22, 2, 14).setZero(); // exculde arm joints
        ...
        kin_tasks_stand.taskLib[id].J.block(2, 20, 3, 2).setZero();  // exculde waist joints
        kin_tasks_stand.taskLib[id].J.block(2, 22, 3, 14).setZero(); // exculde arm joints
        ...
        kin_tasks_stand.taskLib[id].W.diagonal()(20) = 200; // 14+6: waist yaw
        kin_tasks_stand.taskLib[id].W.diagonal()(21) = 200; // 15+6: waist pitch
        ...
        kin_tasks_stand.taskLib[id].errX(0) = 0 - q(21); // 14+7: waist yaw
        kin_tasks_stand.taskLib[id].errX(1) = 0 - q(22); // 15+7: waist pitch

        kin_tasks_stand.taskLib[id].derrX = Eigen::VectorXd::Zero(2);
        kin_tasks_stand.taskLib[id].ddxDes = Eigen::VectorXd::Zero(2);
        kin_tasks_stand.taskLib[id].dxDes = Eigen::VectorXd::Zero(2);
        kin_tasks_stand.taskLib[id].kp = Eigen::MatrixXd::Identity(2, 2) * 200;
        kin_tasks_stand.taskLib[id].kd = Eigen::MatrixXd::Identity(2, 2) * 20;
        kin_tasks_stand.taskLib[id].J = Eigen::MatrixXd::Zero(2, model_nv);

        kin_tasks_stand.taskLib[id].J(0, 20) = 1;
        kin_tasks_stand.taskLib[id].J(0, 21) = 1;
...
void WBC_priority_HRO::copy_Eigen_to_real_t(qpOASES::real_t *target, const Eigen::MatrixXd &source, int nRows, int nCols)
{
    int count = 0;
...
void WBC_priority_HRO::setQini(const Eigen::VectorXd &qIniDesIn, const Eigen::VectorXd &qIniCurIn)
{
    qIniDes = qIniDesIn;
    qIniCur = qIniCurIn;
}