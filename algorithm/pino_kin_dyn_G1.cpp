...
Pin_KinDyn_G1::Pin_KinDyn_G1(std::string urdf_pathIn)
{
    ...
    r_hip_joint = model_biped.getJointId("right_hip_pitch_joint");
    l_hip_joint = model_biped.getJointId("left_hip_pitch_joint");
    ...
    r_hip_joint_fixed = model_biped_fixed.getJointId("right_hip_pitch_joint");
    l_hip_joint_fixed = model_biped_fixed.getJointId("left_hip_pitch_joint");
    ...
    std::ifstream in("joint_ctrl_config_G1.json", std::ios::binary);
...
void Pin_KinDyn_G1::dataBusRead(const DataBus &robotState)
{
...
void Pin_KinDyn_G1::dataBusWrite(DataBus &robotState)
{
    robotState.J_l = J_l;
    robotState.J_r = J_r;
...
void Pin_KinDyn_G1::computeJ_dJ()
{
...
Eigen::Quaterniond Pin_KinDyn_G1::intQuat(const Eigen::Quaterniond &quat, const Eigen::Matrix<double, 3, 1> &w)
{
...
Eigen::VectorXd Pin_KinDyn_G1::integrateDIY(const Eigen::VectorXd &qI, const Eigen::VectorXd &dqI)
{
...
// Pin_KinDyn_G1::IkRes
// Pin_KinDyn_G1::computeInK_Leg(const Eigen::Matrix3d &Rdes_L, const Eigen::Vector3d &Pdes_L, const Eigen::Matrix3d &Rdes_R,
...
// Pin_KinDyn_G1::IkRes
// Pin_KinDyn_G1::computeInK_Hand(const Eigen::Matrix3d &Rdes_L, const Eigen::Vector3d &Pdes_L, const Eigen::Matrix3d &Rdes_R,
//                                const Eigen::Vector3d &Pdes_R)
// {
//     const pinocchio::SE3 oMdesL(Rdes_L, Pdes_L);
//     const pinocchio::SE3 oMdesR(Rdes_R, Pdes_R);
                              ...
                              const Eigen::Vector3d &Pdes_R)
{
    ...
    Eigen::VectorXd qIk = Eigen::VectorXd::Zero(model_biped_fixed.nv); // initial guess
    qIk[3] = +0.2;                                                     // -0.1;                                                     // left knee swing back
    qIk[9] = +0.2;                                                     // -0.1;                                                     // right knee
    ...
    for (itr_count = 0;; itr_count++)
    {
        ...
        // shoulder-l: 13-15, elbow-l: 16, wrist-roll-l: 17,
        // shoulder-r: 18-20, elbow-r: 21, wrist-roll-r: 22
        W(12, 12) = 0.0001;              // use a smaller value to make the solver try not to use waist joint
        JL.block(0, 12, 6, 1).setZero(); // 12: waist
        JR.block(0, 12, 6, 1).setZero(); // 12: waist
                               ...
                               const Eigen::Vector3d &Pdes_R)
{
    ...
    // shoulder-l: 13-15, elbow-l: 16, wrist-roll-l: 17,
    // shoulder-r: 18-20, elbow-r: 21, wrist-roll-r: 22
    qIk.block<5, 1>(13, 0) << 0.433153883479341,
        0.802378252758275, -0.356, 0.0, -0.0;

    qIk.block<5, 1>(18, 0) << -0.433152540054138, 0.802375980602373, 0.356, 0.0, 0.0;
...
void Pin_KinDyn_G1::workspaceConstraint(Eigen::VectorXd &qFT, Eigen::VectorXd &tauJointFT)
{
    for (int i = 0; i < motorName.size(); i++)
