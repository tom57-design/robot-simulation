...
Pin_KinDyn_HRO::Pin_KinDyn_HRO(std::string urdf_pathIn)
{
    ...
    r_ankle_joint = model_biped.getJointId("right_leg_joint_7");
    l_ankle_joint = model_biped.getJointId("left_leg_joint_7");
    r_hand_joint = model_biped.getJointId("right_arm_joint_7");
    ...
    std::ifstream in("joint_ctrl_config_HRO.json", std::ios::binary);
...
void Pin_KinDyn_HRO::dataBusWrite(DataBus &robotState)
{
    robotState.J_l = J_l;
    robotState.J_r = J_r;
...
void Pin_KinDyn_HRO::computeJ_dJ()
{
...
Eigen::Quaterniond Pin_KinDyn_HRO::intQuat(const Eigen::Quaterniond &quat, const Eigen::Matrix<double, 3, 1> &w)
{
...
Eigen::VectorXd Pin_KinDyn_HRO::integrateDIY(const Eigen::VectorXd &qI, const Eigen::VectorXd &dqI)
{
...
void Pin_KinDyn_HRO::computeDyn()
{
    // cal M
...
Pin_KinDyn_HRO::IkRes
Pin_KinDyn_HRO::computeInK_Leg(const Eigen::Matrix3d &Rdes_L, const Eigen::Vector3d &Pdes_L, const Eigen::Matrix3d &Rdes_R,
...
Pin_KinDyn_HRO::IkRes
Pin_KinDyn_HRO::computeInK_Hand(const Eigen::Matrix3d &Rdes_L, const Eigen::Vector3d &Pdes_L, const Eigen::Matrix3d &Rdes_R,
                                const Eigen::Vector3d &Pdes_R)
{
    const pinocchio::SE3 oMdesL(Rdes_L, Pdes_L);
    const pinocchio::SE3 oMdesR(Rdes_R, Pdes_R);
...
void Pin_KinDyn_HRO::workspaceConstraint(Eigen::VectorXd &qFT, Eigen::VectorXd &tauJointFT)
{
    for (int i = 0; i < motorName.size(); i++)
