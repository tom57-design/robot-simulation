...
MJ_interface_G1::MJ_interface_G1(mjModel *mj_modelIn, mjData *mj_dataIn) {
    mj_model=mj_modelIn;
    mj_data=mj_dataIn;
...
void MJ_interface_G1::updateSensorValues() {
    for (int i=0;i<jointNum;i++){
...
void MJ_interface_G1::setMotorsTorque(std::vector<double> &tauIn) {
    for (int i=0;i<jointNum;i++)
        mj_data->ctrl[i]=tauIn.at(i);
}

void MJ_interface_G1::dataBusWrite(DataBus &busIn) {
